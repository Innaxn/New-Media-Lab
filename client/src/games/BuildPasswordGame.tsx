...
import {
  Box, Typography, TextField, IconButton, Alert, Button,
  Fade, Card, CardContent, Tooltip,
} from '@mui/material';
...
interface Props { questions: BuildPasswordQuestion[]; date: string; onBack: () => void; }

function PasswordLevel({ q, onComplete }: { q: BuildPasswordQuestion; onComplete: () => void }) {
  ...
  const ruleResults = q.rules.map(r => ({ rule: r, passed: evaluateRule(password, r) }));
    ...
    <Box className="slide-up">
      ...
      <Card elevation={2} sx={{ mb: 2 }}>
        <CardContent>
          ...
          <Box sx={{ display: 'flex', gap: 0.5, mb: 0.75 }}>
            {[0,1,2,3,4].map(i => (
              <Box key={i} sx={{
                ...
                bgcolor: password.length > 0 && i < strength.level ? strength.color : p.border,
                boxShadow: password.length > 0 && i < strength.level ? `0 0 5px ${alpha(strength.color, 0.5)}` : 'none',
                transition: 'all 0.3s',
              }} />
            ))}
          </Box>
          <Typography variant="caption" sx={{ display: 'block', mb: 2, color: password.length === 0 ? 'text.disabled' : strength.color, fontWeight: 600 }}>
            {strength.label}
          </Typography>

          {/* Checklist — rules from backend */}
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0.875, mb: 2 }}>
            {ruleResults.map(({ rule, passed }) => (
              <Box key={rule.description} sx={{ display: 'flex', alignItems: 'center', gap: 0.75, color: passed ? p.primary : p.inkGhost, transition: 'color 0.2s' }}>
                {passed
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: '10px 14px', bgcolor: 'background.default', border: `1px solid ${p.border}`, borderRadius: '8px' }}>
            <Typography variant="caption" sx={{ color: 'text.disabled' }}>How long to crack?</Typography>
            <Typography variant="caption" sx={{ fontFamily: 'monospace', fontSize: '0.875rem', fontWeight: 600, color: strength.level >= 3 ? p.primary : p.danger, transition: 'color 0.3s' }}>
              {strength.crackTime}
            </Typography>
          </Box>
        </CardContent>
...
export default function BuildPasswordGame({ questions, date, onBack }: Props) {
  ...
  if (activeLevel) {
    ...
    return (
      <GameShell
        title="Build a Password" difficulty={activeLevel} date={date}
        progress={completed.has(activeLevel) ? 100 : undefined}
        onBack={() => setActiveLevel(null)}
        infoTitle="Why do passwords matter?"
        infoContent={INFO_TEXT}
      >
        <Typography variant="overline" sx={{ display: 'block', mb: 1, color: p.primary }}>
        ...
        <Typography variant="body2" sx={{ mb: 3 }}>Make a password that passes all the checks below. A new set of checks appears every day.</Typography>
  ...
  if (allDone) {
    return (
      <GameShell title="Build a Password" difficulty="hard" date={date} progress={100} onBack={onBack} infoTitle="Why do passwords matter?" infoContent={INFO_TEXT}>
        <Box sx={{ textAlign: 'center', py: 8 }} className="slide-up">
          <EmojiEventsIcon sx={{ fontSize: 56, color: p.primary, mb: 2, filter: `drop-shadow(0 0 12px ${alpha(p.primary, 0.5)})` }} />
          <Typography variant="h2" sx={{ mb: 1 }}>All levels done! 🎉</Typography>
          <Typography variant="body2" sx={{ mb: 3, color: 'text.secondary' }}>Come back tomorrow for a new password challenge.</Typography>
          <Alert severity="success" sx={{ maxWidth: 420, mx: 'auto', textAlign: 'left' }}>
            Remember: longer passwords win. A random 16-character password is almost impossible to crack — even for the fastest computers.
          </Alert>
        </Box>
      </GameShell>
    );
  ...
  return (
    <GameShell title="Build a Password" difficulty="easy" date={date} onBack={onBack} infoTitle="Why do passwords matter?" infoContent={INFO_TEXT}>
      <Typography variant="overline" sx={{ display: 'block', mb: 1, color: p.primary }}>Today's Challenge</Typography>
      <Typography variant="h2" sx={{ mb: 1 }}>Build a Password</Typography>
      <Typography variant="body2" sx={{ mb: 4, maxWidth: 520 }}>
        Three levels — starting easy, getting harder. Each level gives you a checklist to hit. Complete them in order to unlock the next.
      </Typography>
      <LevelPicker
        levels={sortedQ.map(q => ({ difficulty: q.difficulty, subtitle: `${q.rules.length} requirements to meet` }))}
        completed={completed}
