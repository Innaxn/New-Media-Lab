...
export default function App() {
  ...
  const renderGame = (game: GameOfTheDay) => {
    ...
    switch (question_type) {
      case 'build_a_password':
        return <BuildPasswordGame questions={questions as BuildPasswordQuestion[]} date={date} onBack={goHome} />;

      case 'spot_the_weakest_password':
        return <SpotWeakestGame questions={questions as SpotWeakestQuestion[]} date={date} onBack={goHome} />;

      case 'build_a_passphrase':
        return <BuildPassphraseGame questions={questions as BuildPassphraseQuestion[]} date={date} onBack={goHome} />;

      case 'phish_or_legit':
        return <PhishingGame questions={questions as PhishOrLegitQuestion[]} date={date} onBack={goHome} />;

      case 'multiple_choice':
        return <MultipleChoiceGame questions={questions as MultipleChoiceQuestion[]} date={date} onBack={goHome} />;
  ...
  if (state.view === 'loading') {
    return (
      <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'background.default' }}>
        <Container maxWidth="sm"><LoadingState status="loading" minHeight={280} /></Container>
      </Box>
    );
  }
  ...
  if (state.view === 'error') {
    return (
      <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: 'background.default' }}>
        <Container maxWidth="sm"><LoadingState status="error" error={state.message} onRetry={loadGame} minHeight={280} /></Container>
      </Box>
    );
  }
